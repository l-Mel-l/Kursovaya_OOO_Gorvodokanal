...
public class DataBase {
    public static void createDB() {
        try (
            ...
            statement.execute("CREATE TABLE IF NOT EXISTS Заявление (Id INTEGER PRIMARY KEY, ФИО VARCHAR, Телефон INTEGER, Адрес VARCHAR,ДатаРождения VARCHAR, Почта  VARCHAR, Заявка VARCHAR)");
    ...
    public static void NewZayav( String loginText, String numberText, String adressText, String fioText, String dataText,String zayavText) {
        try (
                Connection connection = DriverManager.getConnection("jdbc:sqlite:" + "Vodokanal.db");
                Statement statement = connection.createStatement()) {
            statement.execute("INSERT INTO Заявление (ФИО, Телефон, Адрес, ДатаРождения, Почта, Заявка) VALUES (\"" + fioText + "\",\"" + numberText + "\",\"" + adressText + "\",\"" + dataText + "\",\"" + loginText + "\",\"" + zayavText + "\")");
